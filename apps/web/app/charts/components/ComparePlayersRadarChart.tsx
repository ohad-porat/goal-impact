...
function CustomLegend({ player1Name, player2Name }: { player1Name: string; player2Name: string }) {
  return (
    <div className="flex items-center justify-center gap-8">
      <div className="flex items-center gap-2">
        <div className="w-3 h-3 rounded-sm" style={{ backgroundColor: PLAYER1_COLOR }} />
        <span className="text-sm text-gray-300">{player1Name}</span>
      </div>
      <div className="flex items-center gap-2">
        <div className="w-3 h-3 rounded-sm" style={{ backgroundColor: PLAYER2_COLOR }} />
...
export function ComparePlayersRadarChart() {
  ...
  useEffect(() => {
    const fetchPlayerDetails = async (
      ...
      setPlayer((prev: PlayerData) => ({ ...prev, loading: true, error: null }))
      try {
        const response = await fetch(api.playerDetails(playerId), { cache: 'no-cache' })
        if (!response.ok) {
  ...
  }, [player1.player, player1.playerDetails, player1.loading, player2.player, player2.playerDetails, player2.loading])

  const createPlayerSelectHandler = (setPlayer: React.Dispatch<React.SetStateAction<PlayerData>>) => 
    (player: SearchResult | null) => {
      if (!player) {
        setPlayer({
          player: null,
  ...
  const normalizedChartData = useMemo(() => {
    ...
    return chartData.map((point) => {
      const metricKey = METRICS.find((m) => m.label === point.metric)?.key || ''
  ...
  return (
    <div>
      <h2 className="text-xl sm:text-2xl font-bold text-white mb-6">Compare Players</h2>
        ...
        <div className="space-y-4">
          ...
          {player1.playerDetails && (
            <div>
              ...
              <select
                value={player1.selectedSeasonId === 'career' ? 'career' : player1.selectedSeasonId || ''}
                onChange={(e) => {
                  const value = e.target.value
                  setPlayer1((prev) => ({
                    ...prev,
                    selectedSeasonId: value === 'career' ? 'career' : parseInt(value, 10),
                  }))
                }}
              ...
              >
                ...
                {player1.playerDetails.seasons.map((season, index) => (
                  <option key={`player1-${season.season.id}-${index}`} value={season.season.id}>
        ...
        <div className="space-y-4">
          ...
          {player2.playerDetails && (
            <div>
              ...
              <select
                value={player2.selectedSeasonId === 'career' ? 'career' : player2.selectedSeasonId || ''}
                onChange={(e) => {
                  const value = e.target.value
                  setPlayer2((prev) => ({
                    ...prev,
              ...
              >
                ...
                {player2.playerDetails.seasons.map((season, index) => (
                  <option key={`player2-${season.season.id}-${index}`} value={season.season.id}>
                    {season.season.display_name}
                  </option>
                ))}
              </select>
      ...
      {canRenderChart ? (
        <div className="bg-slate-800 rounded-lg p-4 max-w-2xl mx-auto" data-testid="compare-players-radar-chart">
          <CustomLegend 
            player1Name={player1.player?.name || 'Player 1'}
            player2Name={player2.player?.name || 'Player 2'}
          />
          <ResponsiveContainer width="100%" height={500}>
            <RadarChart data={normalizedChartData} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
              ...
              <Tooltip
                ...
                formatter={(value: number | undefined, name: string | undefined, props: any) => {
                  ...
                  if (!metricLabel) return [value?.toFixed(1) || '0', playerName]
                  
                  const originalMetric = METRICS.find((m) => m.label === metricLabel)
                  if (!originalMetric) return [value?.toFixed(1) || '0', playerName]

                  const metricKey = originalMetric.key
                  let originalValue = 0
                  
                  if (playerName === player1.player?.name) {
                    originalValue = player1Stats?.[metricKey as keyof typeof player1Stats] || 0
                  } else if (playerName === player2.player?.name) {
                    originalValue = player2Stats?.[metricKey as keyof typeof player2Stats] || 0
                  }
