...
export function useLeaderFilters() {
  ...
  const seasonId = selectedSeasonId && !isNaN(parseInt(selectedSeasonId, 10))
    ? parseInt(selectedSeasonId, 10)
    : undefined

  const updateParams = (view: string, updates: Record<string, string | null>) => {
  ...
  return { leagueId, seasonId, selectedLeagueId, selectedSeasonId, updateParams }
}
