...
vi.mock("../../../lib/api", () => ({
  api: {
    leagueSeasons: (leagueId: number) =>
      `https://api.example.com/leagues/${leagueId}/seasons`,
...
describe("useSeasons", () => {
  ...
  it("should fetch all seasons from allSeasons endpoint when leagueId is undefined", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(api.allSeasons, {
      cache: "no-cache",
    });
  ...
  it("should fetch league-specific seasons from leagueSeasons endpoint when leagueId is provided", async () => {
    ...
    expect(result.current.seasons).toEqual(mockSeasons);
    expect(global.fetch).toHaveBeenCalledWith(api.leagueSeasons(5), {
      cache: "no-cache",
    });
    expect(global.fetch).toHaveBeenCalledTimes(1);
  });
  ...
  it("should refetch when leagueId changes from 5 to 7 and update seasons accordingly", async () => {
    ...
    const { result, rerender } = renderHook(
      ({ leagueId }: { leagueId?: number }) => useSeasons(leagueId),
      {
    ...
    expect(global.fetch).toHaveBeenCalledWith(api.leagueSeasons(7), {
      cache: "no-cache",
    });
