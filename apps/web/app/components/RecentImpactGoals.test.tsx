...
describe("RecentImpactGoals", () => {
  ...
  describe("Initial rendering", () => {
    it("should render top 4 league filter buttons", () => {
      ...
      expect(
        screen.getByRole("button", { name: "Serie A" }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: "La Liga" }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: "Bundesliga" }),
      ).toBeInTheDocument();
      expect(
        screen.queryByRole("button", { name: "Ligue 1" }),
    ...
    it('should highlight "All Leagues" button by default', () => {
      ...
      const allLeaguesButton = screen.getByRole("button", {
        name: "All Leagues",
      });
  ...
  describe("Goal display", () => {
    ...
    it("should handle goals without score information", () => {
      ...
      render(
        <RecentImpactGoals
          initialLeagues={mockLeagues}
          initialGoals={goalsWithoutScore}
        />,
      );
  ...
  describe("League filtering", () => {
    it("should filter and sort top leagues correctly", () => {
      ...
      render(
        <RecentImpactGoals initialLeagues={leagues} initialGoals={mockGoals} />,
      );

      const buttons = screen.getAllByRole("button");
      const leagueButtons = buttons.filter(
        (btn) => btn.textContent !== "All Leagues",
      );
    ...
    it('should reset to "All Leagues" when clicking All Leagues button', async () => {
      ...
      const premierLeagueButton = screen.getByRole("button", {
        name: "Premier League",
      });
      const allLeaguesButton = screen.getByRole("button", {
        name: "All Leagues",
  ...
  describe("Error handling", () => {
    it("should display error message when fetch fails", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Failed to load recent goals."),
        ).toBeInTheDocument();
  ...
  describe("Empty state", () => {
    ...
    it("should render nothing when initialGoals is null", () => {
      render(
        <RecentImpactGoals initialLeagues={mockLeagues} initialGoals={null} />,
      );

      expect(screen.getByText("Recent Impact Goals")).toBeInTheDocument();
      expect(
        screen.queryByText("No recent goals found"),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText("Failed to load recent goals."),
      ).not.toBeInTheDocument();
    });
  ...
  describe("Edge cases", () => {
    ...
    it("should handle multiple goal value formats", () => {
      ...
      render(
        <RecentImpactGoals
          initialLeagues={mockLeagues}
          initialGoals={goalsWithVariousValues}
        />,
      );
