...
export function TeamSeasonGoalLogTableBody({ goals }: TeamSeasonGoalLogTableBodyProps) {
      ...
      <tbody className="bg-slate-800 divide-y divide-gray-700">
        <tr>
          <td colSpan={10} className="px-6 py-8 text-center">
            <p className="text-gray-400 text-lg">No goals found for this season</p>
          </td>
        </tr>
      </tbody>
    )
  ...
  return (
    <tbody className="bg-slate-800">
      {goals.map((goal, index) => {
        ...
        const borderClass = isDateChange ? 'border-t-2 border-gray-500' : index > 0 ? 'border-t border-gray-700' : ''
