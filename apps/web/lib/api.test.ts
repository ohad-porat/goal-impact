...
describe("API Client", () => {
  ...
  describe("Path parameter endpoints", () => {
    it("should construct leagueSeasons with leagueId", () => {
      expect(api.leagueSeasons(123)).toBe(
        `${mockApiBaseUrl}/leagues/123/seasons`,
      );
    });

    it("should construct leagueTable with leagueId and seasonId", () => {
      expect(api.leagueTable(5, 10)).toBe(
        `${mockApiBaseUrl}/leagues/5/table/10`,
      );
    });
    ...
    it("should construct teamSeasonSquad with teamId and seasonId", () => {
      expect(api.teamSeasonSquad(7, 15)).toBe(
        `${mockApiBaseUrl}/clubs/7/seasons/15`,
      );
    });

    it("should construct teamSeasonGoalLog with teamId and seasonId", () => {
      expect(api.teamSeasonGoalLog(8, 20)).toBe(
        `${mockApiBaseUrl}/clubs/8/seasons/20/goals`,
      );
    });
    ...
    it("should construct playerGoalLog with playerId", () => {
      expect(api.playerGoalLog(101)).toBe(
        `${mockApiBaseUrl}/players/101/goals`,
      );
    });
  ...
  describe("Query parameter endpoints", () => {
    it("should construct leadersCareerTotals without leagueId", () => {
      expect(api.leadersCareerTotals()).toBe(
        `${mockApiBaseUrl}/leaders/career-totals`,
      );
    });

    it("should construct leadersCareerTotals with leagueId", () => {
      expect(api.leadersCareerTotals(5)).toBe(
        `${mockApiBaseUrl}/leaders/career-totals?league_id=5`,
      );
    });

    it("should construct leadersBySeason with seasonId only", () => {
      expect(api.leadersBySeason(10)).toBe(
        `${mockApiBaseUrl}/leaders/by-season?season_id=10`,
      );
    });

    it("should construct leadersBySeason with seasonId and leagueId", () => {
      expect(api.leadersBySeason(10, 5)).toBe(
        `${mockApiBaseUrl}/leaders/by-season?season_id=10&league_id=5`,
      );
    });
    ...
    it("should construct recentGoals with leagueId", () => {
      expect(api.recentGoals(7)).toBe(
        `${mockApiBaseUrl}/home/recent-goals?league_id=7`,
      );
    });
  });
  ...
  describe("Edge cases", () => {
    it("should handle leagueId as 0 in query params", () => {
      expect(api.leadersCareerTotals(0)).toBe(
        `${mockApiBaseUrl}/leaders/career-totals?league_id=0`,
      );
    });
  });
