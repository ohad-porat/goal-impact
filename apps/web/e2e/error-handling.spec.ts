...
test.describe('Error Handling', () => {
  test.describe('Invalid IDs', () => {
    for (const { type, path } of resourceTypes) {
      test(`should display error for invalid ${type} ID`, async ({ page }) => {
        ...
        await verifyErrorMessage(page, new RegExp(`The ${type} ID "invalid" is not valid`, 'i'));
      });
    }

    for (const { type, path } of resourceTypes) {
      test(`should display error for various non-numeric ${type} ID formats`, async ({ page }) => {
        const invalidIds = ['abc', 'test-123', 'null', 'undefined'];
        
        for (const invalidId of invalidIds) {
          await navigateAndWait(page, `${path}/${invalidId}`);
          await verifyErrorMessage(page, new RegExp(`The ${type} ID "${invalidId}" is not valid`, 'i'));
        }
      });
    }
  });
  ...
  test.describe('Non-existent Resources', () => {
    for (const { type, path } of resourceTypes) {
      test(`should display error for non-existent ${type}`, async ({ page }) => {
        await navigateAndWait(page, `${path}/999999`);
        await verifyErrorMessage(page, new RegExp(`The requested ${type} could not be found or does not exist`, 'i'));
      });
    }
  });
  ...
  test.describe('Missing or Invalid Season Parameters', () => {
    ...
    for (const { name, path } of clubSeasonPaths) {
      test(`should display error for missing season parameter in ${name}`, async ({ page }) => {
        await navigateAndWait(page, path);
        await verifyErrorMessage(page, /Season parameter is required/i);
      });

      test(`should display error for invalid season parameter in ${name}`, async ({ page }) => {
  ...
  test.describe('Invalid Season Selection', () => {
    test('should display error for invalid season in league page', async ({ page }) => {
      await navigateAndWait(page, '/leagues/1?season=999999');
      await verifyErrorMessage(page, /The requested season is not valid/i);
    });

    test('should display error for invalid season in player goal log', async ({ page }) => {
  ...
  test.describe('Error Recovery', () => {
    ...
    test('should allow navigation to valid page from error', async ({ page }) => {
      await navigateAndWait(page, '/nations/invalid');
      await expect(page.getByText(/Error:/i)).toBeVisible();
