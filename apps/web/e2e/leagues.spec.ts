...
test.describe("Leagues", () => {
  test.describe("List Page", () => {
    ...
    test("should display leagues in table with clickable links", async ({
      page,
    }) => {
  ...
  test.describe("Detail Page", () => {
    ...
    test.beforeEach(async ({ page }) => {
      ...
      await expect(page).toHaveURL(
        new RegExp(`/leagues/${leagueId}(\\?season=\\d+)?$`),
      );
    ...
    test.describe("Season Selector", () => {
      ...
      test("should change season when selecting different option", async ({
        ...
        await page.waitForURL(
          new RegExp(`/leagues/${leagueId}\\?season=${lastOptionValue}`),
          { timeout: 5000 },
        );
        await page.waitForLoadState("networkidle");
      ...
      test("should update league table when season changes", async ({
        page,
      }) => {
        expect(leagueId).not.toBeNull();
        await expect(page).toHaveURL(
  ...
  test.describe("Navigation Flow", () => {
    test("should navigate from list to detail and back", async ({ page }) => {
      ...
      const navigationPromise = page.waitForURL("**/leagues", {
        timeout: 10000,
      });
      await page.goBack();
    ...
    test("should include season parameter in URL on detail page", async ({
      page,
    }) => {
      ...
      await page.waitForURL(
        (url) => url.toString().includes(`season=${firstOptionValue}`),
        { timeout: 5000 },
      );
      await page.waitForLoadState("networkidle");
