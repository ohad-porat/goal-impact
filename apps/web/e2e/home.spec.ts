...
test.describe("Home Page", () => {
  ...
  test.describe("Recent Impact Goals", () => {
    test("should display league filter buttons on desktop", async ({
      page,
    }) => {
      const allLeaguesButton = page.getByRole("button", {
        name: "All Leagues",
      });
    ...
    test("should display league filter dropdown on mobile", async ({
      page,
    }) => {
    ...
    test("should filter goals by league when clicking league button", async ({
      page,
    }) => {
    ...
    test("should reset to all leagues when clicking All Leagues button", async ({
      page,
    }) => {
      await page.waitForLoadState("networkidle");
    ...
    test("should display goals when available", async ({ page }) => {
      ...
      if (hasError) {
        throw new Error(
          "Failed to load recent goals - test cannot verify goals are displayed",
        );
      }
  ...
  test.describe("How It Works Section", () => {
    test("should display Goal Value explanation", async ({ page }) => {
      await expect(
        page.getByText(/Goal Value measures how much a goal changes/i),
      ).toBeVisible();
    });
    ...
    test("should display Goal Value example explanation", async ({ page }) => {
      await expect(
        page.getByText(/A Goal Value of \+0\.64 means/i),
      ).toBeVisible();
    });
  });
});
