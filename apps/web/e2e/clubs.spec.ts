...
async function navigateToSeasonRoster(page: Page): Promise<number> {
...
test.describe("Clubs", () => {
  test.describe("List Page", () => {
    ...
    test("should display clubs organized by nation", async ({ page }) => {
      ...
      await verifyEmptyStateOrContent(
        page,
        "No clubs data available",
        clubLinks,
      );

      const nationHeadings = page
        .locator("h2")
  ...
  test.describe("Detail Page", () => {
    ...
    test("should display club name and nation", async ({ page }) => {
      ...
      const nationText = page
        .locator("p")
        .filter({ hasText: /England|Spain|Unknown Nation/ });
  ...
  test.describe("Season Roster Page", () => {
    ...
    test("should navigate from detail to season roster", async ({ page }) => {
      ...
      await expect(page).toHaveURL(
        `/clubs/${clubId}/seasons?season=${seasonId}`,
      );
    });

    test("should display season roster heading with club and season info", async ({
      page,
    }) => {
      expect(clubId).not.toBeNull();
      expect(seasonId).not.toBeNull();
  ...
  test.describe("Goal Log Page", () => {
    ...
    test("should navigate from season roster to goal log", async ({ page }) => {
      ...
      expect(seasonId).not.toBeNull();
      await expect(page).toHaveURL(
        `/clubs/${clubId}/seasons/goals?season=${seasonId}`,
      );
    });

    test("should display goal log heading with club and season info", async ({
      page,
    }) => {
      expect(clubId).not.toBeNull();
      expect(seasonId).not.toBeNull();
    ...
    test('should navigate back to roster when clicking "Back to Roster"', async ({
      page,
    }) => {
      expect(clubId).not.toBeNull();
      expect(seasonId).not.toBeNull();
  ...
  test.describe("Navigation Flow", () => {
    test("should navigate through complete flow: list → detail → roster → goal log → back", async ({
      page,
    }) => {
