...
async function hasNoGoals(page: Page, emptyStateText: string = 'No goals found for this player'): Promise<boolean> {
  const noGoalsText = page.getByText(emptyStateText);
  return await noGoalsText.isVisible().catch(() => false);
}

async function navigateToPlayerViaSearch(page: Page, searchQuery: string = 'test'): Promise<number> {
  ...
  test.describe('Goal Log Page', () => {
    ...
    test.describe('Season Selector', () => {
      ...
      test('should change season when selecting different option', async ({ page }) => {
        ...
        await page.waitForURL(new RegExp(`/players/${playerId}/goals\\?season=${lastOptionValue}`), { timeout: 5000 });
        await page.waitForLoadState('networkidle');
