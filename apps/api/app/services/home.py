...
from app.schemas.home import RecentImpactGoal, RecentGoalMatch, RecentGoalPlayer


def get_recent_impact_goals(db: Session, days: int = 7, limit: int = 5, league_id: Optional[int] = None) -> List[RecentImpactGoal]:
    ...
    ).order_by(desc(Event.goal_value)).limit(limit).all()
    ...
    for goal_event, match_obj, player_obj, home_team_obj, away_team_obj in goals_query:
        ...
        if (goal_event.home_team_goals_pre_event is not None and 
            goal_event.away_team_goals_pre_event is not None and
            goal_event.home_team_goals_post_event is not None and 
            goal_event.away_team_goals_post_event is not None):
