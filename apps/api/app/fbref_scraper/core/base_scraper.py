...
from .scraper_config import ScraperConfig, get_config, get_rate_limit

T = TypeVar("T")
FAILED_RECORDS_FILE = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "logs", "failed_records.txt"
)
...
class BaseScraper(ABC):
    ...
    def fetch_page(
        self, url: str, sleep_time: int | None = None, max_retries: int = 5
    ) -> BeautifulSoup:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if response.status_code == 403:
                    ...
                    self.logger.warning(
                        f"Forbidden (403) on attempt {attempt + 1}/{max_retries + 1} for {url}"
                    )
                    ...
                    if attempt < max_retries:
                        ...
                        self.logger.error(
                            f"Failed after {max_retries + 1} attempts with 403 Forbidden"
                        )
                ...
                if attempt == max_retries:
                    self.logger.error(
                        f"HTTP Error fetching {url} after {max_retries + 1} attempts: {e}"
                    )
                    raise

                wait_time = sleep_time * (2**attempt)
                self.logger.warning(
                    f"Request failed (attempt {attempt + 1}/{max_retries + 1}). Waiting {wait_time}s: {e}"
                )
                time.sleep(wait_time)
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if response.status_code == 403:
                    wait_time = sleep_time * (2**attempt)
                    self.logger.warning(
                        f"Forbidden (403) fetching HTML table on attempt {attempt + 1}/{max_retries + 1}"
                    )
                    if attempt < max_retries:
                        self.logger.warning(f"Waiting {wait_time}s before retry")
                        time.sleep(wait_time)
                ...
                if attempt == max_retries:
                    self.logger.error(
                        f"Error fetching HTML table from {url} after {max_retries + 1} attempts: {e}"
                    )
                    raise

                wait_time = sleep_time * (2**attempt)
                self.logger.warning(
                    f"HTML table fetch failed (attempt {attempt + 1}/{max_retries + 1}). Waiting {wait_time}s: {e}"
                )
                time.sleep(wait_time)
    ...
    def log_error_and_continue(
        self, operation: str, error: Exception, entity_name: str = ""
    ) -> None:
        """Log an error but continue processing instead of stopping."""
        self.logger.error(
            f"Error in {operation} for {entity_name}: {error}. Continuing with next item."
        )
    ...
    def _get_progress_file(self) -> str:
        ...
        return os.path.join(
            ...
            "logs",
            f"progress_{self.__class__.__name__.lower()}_{timestamp}.json",
        )
    ...
    def log_failed_record(self, record_type: str, record_identifier: str, error: str) -> None:
        ...
        try:
            ...
            with open(FAILED_RECORDS_FILE, "a") as f:
                f.write(
                    f"[{timestamp}] {scraper_name} - {record_type}: {record_identifier} - Error: {error}\n"
                )
        except Exception as e:
    ...
    def get_failed_records(self) -> list[str]:
        ...
        try:
            if os.path.exists(FAILED_RECORDS_FILE):
                with open(FAILED_RECORDS_FILE) as f:
                    return f.readlines()
        except Exception as e:
    ...
    def _clean_integer_value(self, value):
        ...
        try:
            ...
            if int_value > 2147483647:
                self.logger.warning(
                    f"Integer value {int_value} exceeds PostgreSQL integer max, setting to None"
                )
                return None
            return int_value
    ...
    def _clean_string_value(self, value) -> str | None:
...
class WebScraper(BaseScraper):
    ...
    def load_page(self, url: str, sleep_time: int | None = None) -> BeautifulSoup:
        """Load a web page and store it in the soup attribute."""
        self.soup = self.fetch_page(url, sleep_time)
        return self.soup

    def find_element(self, tag: str, **attributes) -> BeautifulSoup | None:
    ...
    def get_fbref_competition_name(
        self, db_competition_name: str, season_start_year: int = None
    ) -> str:
    ...
    def find_or_create_record(
        ...
        data_dict: dict[str, str | int | float],
        description: str,
    ) -> T:
