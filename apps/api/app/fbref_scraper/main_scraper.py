...
from app.models import Competition, Event, Match, Nation, Season
...
def run_daily_mode(args):
    ...
    logger.info(
        f"Filtering matches/events for seasons with start_year between {from_year} and {to_year}"
    )
    logger.info("Adding new matches and events...")
    matches_scraper = MatchesScraper()
    matches_scraper.run(
        nations=nations, from_date=from_date, to_date=to_date, from_year=from_year, to_year=to_year
    )
    ...
    matches_in_range = (
        ...
        .filter(Match.date >= from_date, Match.date <= to_date)
        .all()
    )
    ...
    logger.info(
        f"Found {len(competition_ids)} competitions and {len(team_ids)} teams with matches in date range"
    )
    ...
    for competition in competitions:
        ...
        logger.info(
            f"Updating team stats for {competition.name} {db_most_recent.start_year}-{db_most_recent.end_year}"
        )
    ...
    if new_goal_events:
        ...
        if player_combinations:
            logger.info(
                f"Updating player stats for {len(player_combinations)} player-season-team combinations..."
            )
...
def _verify_seasons_match(db_season, fbref_start_year, competition_name):
    ...
    if db_season.start_year != fbref_start_year:
        logger.error(
            f"Season mismatch for {competition_name}: DB has {db_season.start_year}-{db_season.end_year}, FBRef shows {fbref_start_year}-{fbref_start_year + 1}"
        )
    ...
    logger.info(
        f"Seasons match for {competition_name}: {db_season.start_year}-{db_season.end_year}"
    )
