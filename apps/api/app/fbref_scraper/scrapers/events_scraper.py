...
from app.models import Competition, Event, Match, Nation, Player, Season, TeamStats
...
class EventsScraper(WebScraper):
    def scrape(
        ...
        for index, team_stats in enumerate(all_team_stats):
            ...
            try:
                ...
                self.log_progress(
                    f"Processing events for {team_stats.team.name} {team_stats.season.start_year}-{team_stats.season.end_year}"
                )

                if team_stats.goal_logs_url is None:
                    self.log_progress(
                        f"Skipping {team_stats.team.name} {team_stats.season.start_year}-{team_stats.season.end_year} - no goal logs URL"
                    )
                    continue
                ...
                events = self.soup.select(
                    '#goallogs_for tr:not(.spacer):not(:has(th:-soup-contains("Rk")))'
                )
                events_processed = 0
                ...
                for event in events[1:]:
                    ...
                    if match and scoring_player:
                        ...
                        self.log_progress(
                            f"Scraping events from {match.date} {match.home_team.name} vs {match.away_team.name}"
                        )
                        ...
                        if assisting_player_element.text.strip() != "":
                            assisting_player_fbref_id = self.extract_fbref_id(
                                assisting_player_element.find("a")["href"]
                            )
                            assisting_player = (
                                self.session.query(Player)
                                .filter(Player.fbref_id == assisting_player_fbref_id)
                                .first()
                            )
                            if assisting_player:
                ...
                self.log_progress(
                    f"Added {events_processed} events for {team_stats.team.name} {team_stats.season.start_year}-{team_stats.season.end_year}"
                )
    ...
    def _parse_event_data(self, event):
        ...
        if team_scored is None or other_team is None:
            home_goals_pre_event = away_goals_pre_event = home_goals_post_event = (
                away_goals_post_event
            ) = None
        else:
            if venue == "Home":
    ...
    def _extract_match_and_player(self, event):
        ...
        match_fbref_id = self.extract_fbref_id(
            event.find("td", {"data-stat": "date"}).find("a")["href"]
        )
        ...
        scoring_player = (
            self.session.query(Player).filter(Player.fbref_id == scoring_player_fbref_id).first()
        )
