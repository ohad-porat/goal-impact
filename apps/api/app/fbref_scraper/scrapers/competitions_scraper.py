...
class CompetitionsScraper(WebScraper):
    def scrape(self, nations: list[str] | None = None) -> None:
        ...
        for row in country_th_elements:
            ...
            for league_tuple in df_list[0].iterrows():
                ...
                competition_link = self.soup.find(
                    "a", string=f"{league['Competition Name']} ({league['Gender']})"
                )
    ...
    def _persist_competition(
        self, nation: Nation, competition_data, comp_type: str, tier: str, fbref_url: str
    ) -> None:
