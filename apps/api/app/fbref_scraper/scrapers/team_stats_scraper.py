...
from app.models import Competition, Nation, Season, Team, TeamStats
...
class TeamStatsScraper(WebScraper):
    def scrape(
        ...
        query = (
            ...
            .join(Season.competition)
            .join(Competition.nation)
        ...
        for index, season in enumerate(all_seasons):
            ...
            try:
                ...
                self.log_progress(
                    f"Processing team stats for {season.competition.name} {season.start_year}-{season.end_year}"
                )
                ...
                if not df_list:
                    self.log_progress(
                        f"No team stats found for {season.competition.name} {season.start_year}-{season.end_year}"
                    )
                    continue
                ...
                for team_stats_tuple in df_list[0].iterrows():
                    ...
                    team_link = self.soup.select_one(
                        f'td.left[data-stat="team"] a:-soup-contains("{team_stats_data["Squad"]}")'
                    )
                    if not team_link:
                        continue
                    ...
                    team_stats_dict = {
                        ...
                        "matches_played": self._clean_integer_value(
                            team_stats_data.get("MP", None)
                        ),
                        ...
                        "goal_difference": self._clean_integer_value(
                            team_stats_data.get("GD", None)
                        ),
                        "points": self._clean_integer_value(team_stats_data.get("Pts", None)),
                        "points_per_match": self._clean_float_value(
                            team_stats_data.get("Pts/MP", None)
                        ),
                        ...
                        "attendance": self._clean_integer_value(
                            team_stats_data.get("Attendance", None)
                        ),
                    ...
                    if update_mode:
                        ...
                        if existing_team_stats:
                            stats_fields = [
                                k
                                for k in team_stats_dict.keys()
                                if k not in ["team_id", "season_id"]
                            ]
                            ...
                            self.session.commit()
                            self.logger.info(
                                f"Updated team stats: {team_stats_data['Squad']} {season.start_year}-{season.end_year}"
                            )
                        else:
                            ...
                            self.logger.info(
                                f"Created new team stats: {team_stats_data['Squad']} {season.start_year}-{season.end_year}"
                            )
                    elif seasonal_mode:
                        existing_team_stats = (
                            self.session.query(TeamStats)
                        ...
                        if existing_team_stats:
                            if existing_team_stats.fbref_url != fbref_url:
                                ...
                                self.session.commit()
                                self.logger.info(
                                    f"Updated team stats URL: {team_stats_data['Squad']} {season.start_year}-{season.end_year}"
                                )
                        else:
                            self.find_or_create_record(
                                TeamStats,
