...
class TestCompetitionsScraper:
    ...
    def test_scrape_success(self, db_session, mocker):
        ...
        competition_data = pd.DataFrame(
            {"Competition Name": ["Premier League"], "Gender": ["M"], "Tier": ["1st"]}
        )

        scraper.fetch_html_table = mocker.Mock(return_value=[competition_data])

        mocker.patch(
            "app.fbref_scraper.scrapers.competitions_scraper.get_selected_nations",
            return_value=["England", "France"],
        )
        mocker.patch(
            "app.fbref_scraper.scrapers.competitions_scraper.get_rate_limit", return_value=2
        )

        scraper.scrape()

        england_competitions = (
            db_session.query(Competition).filter_by(nation_id=england_nation.id).all()
        )
    ...
    def test_scrape_no_competition_link(self, db_session, mocker):
        ...
        competition_data = pd.DataFrame(
            {"Competition Name": ["Unknown League"], "Gender": ["M"], "Tier": ["1st"]}
        )
    ...
    def test_scrape_filters_non_first_tier(self, db_session, mocker):
        ...
        competition_data = pd.DataFrame(
            {"Competition Name": ["Championship"], "Gender": ["M"], "Tier": ["2nd"]}
        )
    ...
    def test_scrape_filters_women_leagues(self, db_session, mocker):
        ...
        competition_data = pd.DataFrame(
            {"Competition Name": ["WSL"], "Gender": ["F"], "Tier": ["1st"]}
        )
    ...
    def test_persist_competition(self, db_session, mocker):
        ...
        competition_data = pd.Series({"Competition Name": "Premier League", "Gender": "M"})
        ...
        scraper._persist_competition(
            nation, competition_data, "League", "1st", "/en/comps/9/Premier-League/"
        )
