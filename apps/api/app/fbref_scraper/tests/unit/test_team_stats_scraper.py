...
class TestTeamStatsScraper:
    ...
    def test_scrape_success_league_competition(self, mocker, db_session):
        ...
        def mock_select_one(selector):
            if 'Arsenal' in selector:
                mock_link = mocker.Mock()
                mock_link.__getitem__ = mocker.Mock(return_value='/en/squads/18bb7c10/Arsenal-Stats')
                return mock_link
            elif 'Chelsea' in selector:
                mock_link = mocker.Mock()
                mock_link.__getitem__ = mocker.Mock(return_value='/en/squads/206d90db/Chelsea-Stats')
                return mock_link
            return None
        ...
        arsenal_stats = db_session.query(TeamStats).join(Team).filter(Team.name == 'Arsenal').first()
    ...
    def test_scrape_with_country_filtering(self, mocker, db_session):
        ...
        db_session.add_all([england, france, eng_competition, fra_competition, eng_season, fra_season])
        db_session.commit()
        ...
        def mock_read_html(url):
            if "comps/9" in url:
                return [pd.DataFrame({
                    ...
                    'Squad': ['Arsenal'],
                    'MP': [38], 'W': [28], 'D': [5], 'L': [5],
                    'GF': [91], 'GA': [29], 'GD': [62], 'Pts': [89],
                    'Pts/MP': [2.34], 'xG': [85.2], 'xGA': [31.5],
                    'xGD': [53.7], 'xGD/90': [1.41]
                })]
            else:
                return [pd.DataFrame({
                    'Rk': [1],
    ...
    def test_scrape_seasonal_mode_existing_stats(self, mocker, db_session):
        ...
        def mock_select_one(selector):
            if 'Arsenal' in selector:
                mock_link = mocker.Mock()
                mock_link.__getitem__ = mocker.Mock(return_value='/en/squads/18bb7c10/Arsenal-Stats-Updated')
                return mock_link
            return None
