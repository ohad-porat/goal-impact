...
class GoalValueRepository:
        ...
        for minute, score_diff_data in goal_value_dict.items():
            for score_diff, goal_value in score_diff_data.items():
                lookup_records.append(
                    GoalValueLookup(minute=minute, score_diff=score_diff, goal_value=goal_value)
                )
    ...
    def save_metadata(self, total_goals, version="1.0"):
        ...
        metadata = StatsCalculationMetadata(total_goals_processed=total_goals, version=version)
