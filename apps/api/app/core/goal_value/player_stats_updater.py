...
class PlayerStatsGoalValueUpdater:
    ...
    def _aggregate_player_goal_values(self):
        ...
        goal_events_query = (
            ...
            .filter(Event.event_type == "goal", Event.goal_value.isnot(None))
            .all()
        )
        ...
        for player_stat in self.all_player_stats:
            ...
            self.aggregated_data[key] = {"total_goal_value": 0.0, "gv_avg": 0.0, "goal_count": 0}

        print(
            f"Initialized {len(self.aggregated_data)} player-season-team combinations with zero values"
        )
        ...
        for event in goal_events_query:
            ...
            scoring_team_id = get_scoring_team_id(
                home_post, home_pre, away_post, away_pre, home_team_id, away_team_id
            )
        ...
        for _key, data in self.aggregated_data.items():
    ...
    def _print_summary(self):
        ...
        print(
            f"Records with goal_value > 0: {sum(1 for data in self.aggregated_data.values() if data['total_goal_value'] > 0)}"
        )
        print(
            f"Records with goal_value = 0: {sum(1 for data in self.aggregated_data.values() if data['total_goal_value'] == 0)}"
        )
    ...
    def update_player_stats_for_combinations(self, combinations: list):
        ...
        logger.info(
            f"Updating player stats for {len(combinations)} player-season-team combinations..."
        )
        ...
        for player_id, season_id, team_id in combinations:
            try:
                ...
                player_stat = (
                    ...
                    .filter_by(player_id=player_id, season_id=season_id, team_id=team_id)
                    .first()
                )
                ...
                logger.error(
                    f"Error processing player_id={player_id}, season_id={season_id}, team_id={team_id}: {e}"
                )
                self.error_count += 1
                continue
        ...
        if update_data:
            try:
                ...
                for i in range(0, len(update_data), batch_size):
                    ...
                    self.session.commit()
                    logger.info(
                        f"  Updated {min(i + batch_size, len(update_data))}/{len(update_data)} player stats records..."
                    )
            except Exception as e:
