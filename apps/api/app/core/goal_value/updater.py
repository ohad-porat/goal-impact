...
class GoalValueUpdater:
    ...
    def _calculate_single_goal_value(self, goal):
        ...
        if (goal.home_team_goals_pre_event is None or 
            goal.home_team_goals_post_event is None or
            goal.away_team_goals_pre_event is None or
            goal.away_team_goals_post_event is None):
    ...
    def _lookup_win_probability(self, minute, score_diff, goal_id):
        ...
        if lookup_minute in self.goal_value_lookup and score_diff in self.goal_value_lookup[lookup_minute]:
            return self.goal_value_lookup[lookup_minute][score_diff]
        
        if (lookup_minute - 1) in self.goal_value_lookup and score_diff in self.goal_value_lookup[lookup_minute - 1]:
